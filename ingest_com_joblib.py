...
lines_processed = 0
def _add_document_to_store(i, document, bst, start, total_lines):

    if i % 10000 == 0:
        elapsed = dt.now() - start
        total = (elapsed/lines_processed)*total_lines
        time_total = "%02d:%02d:%02d:%02d" % (total.days, total.seconds // 3600, total.seconds // 60 % 60, total.seconds % 60)
        to_finish = total - elapsed
        time_to_finish = "%02d:%02d:%02d:%02d" % (to_finish.days, to_finish.seconds // 3600, to_finish.seconds // 60 % 60, to_finish.seconds % 60)
        log.info(f"Processou {i} artigos deste arquivo, {lines_processed} artigos totais, tempo total: {time_total} tempo para fim {time_to_finish}")

    if not bst.buscar(document.metadata["original_id"]):
        chunks = _split_document(document)
        _add_to_store(chunks)
ele abre várias conexões com o banco e dá problema


        #bst.inserir(document.metadata["original_id"])
...
def _load_all_files(files: list[Path]) -> None:
    ...
    for i, file in enumerate(files):
        ...
        if documents is not None:
            Parallel(n_jobs=-1, verbose=4)(delayed(_add_document_to_store)(i, document, bst, start, total_lines) \
